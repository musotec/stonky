...
//        Log.d("POST", "redraw.value = true")
    ...
    var drawings: List<DrawingGraphable>? = null

    val min get() = data.min
    val max get() = data.max

    val selectionRange = selector {
        // FIXME: needs to be invalidated when data changes... immediately!
//        val first = get(start)!!
//        val last = get(end)!!

        val first = data.start.value
        val last = data.end.value
        ...
        // empty data set should be empty selection
        if (data.size == 0) return@selector LinearDomain(0, 0)
        // if the selection range is unset, then we should use the entire set as the domain.
        if (startPct == 0f && endPct == 0f) return@selector LinearDomain(0, data.size - 1).also { it.redraw = false }

        // adjust start and end points so that our binarySearch looks for relative x values.
        val x0 = (if (last is CandleGraphable && first is CandleGraphable) first.time else first.x).toLong()
        val range = (if (last is CandleGraphable && first is CandleGraphable) last.time - first.time else last.x - x0).toFloat()
        val start = (range * startPct).toLong() + x0    // FIXME: selecting first point needs FLOOR because binarySearch does CIEL
        val end = (range * endPct).toLong() + x0
        ...
        data.use {
            ...
            var closestStartPointIndex =
                if (last is CandleGraphable && first is CandleGraphable)
                    data.binarySearch { g -> if (g is CandleGraphable) g.time.compareTo(start) else -1 }
                else
                    data.binarySearch { pointF -> pointF.x.compareTo(start) }
            var closestEndPointIndex =
                if (last is CandleGraphable && first is CandleGraphable)
                    data.binarySearch { g -> if (g is CandleGraphable) g.time.compareTo(end) else -1 }
                else
                    data.binarySearch { pointF -> pointF.x.compareTo(end) }
