...
import tech.muso.demo.graph.core.DrawingGraphable
...
//                get(adapter.end)
    ...
    private val fillTypeAtom: Atom<Int> = rekoilScope.atom { FillType.DOWN } // TOWARD_ZERO }
...
//                    populateRenderingPath()
    ...
    init {
        rekoilScope.launch {
            // change animator if we are candle
            selector {
                val isCandle = get(adapter.data.isCandleGraph) ?: false
                if (isCandle) {
                    animator.cancel()
                    animator = SlideAnimator(postAnimationGraphables)
                } else {
                    if (animator is SlideAnimator) {
                        animator.cancel()
                        animator = MorphAnimator(postAnimationGraphables)
                    }
                }
            }
...
//                        "${Integer.toHexString(color)}, width: $width")
    ...
    private val rawGraphableList get() = adapter.data   //.getWindow(5)
    private val rawExtraGraphableList get() = adapter.drawings
    private var postAnimationGraphables = FifoList<Graphable>(adapter.data.capacity)
    private var postAnimationExtraGraphables = FifoList<Graphable>(25)

    // animation is changed by a selector
    private var animator: GraphableAnimator = MorphAnimator(postAnimationGraphables)

    private var animateFrame = true

    private fun animateTransition(speed: Float = 1f, onAnimationFinished: () -> Unit) {
        adapter.data.useLock {  // use lock during mutation of graphables
    ...
    //        Log.w("AnimateTransition", "#$identifier creating path animation: RawTransformDestination [ start=${rawGraphableList.first()}, end=${rawGraphableList.last()} ]")
            ...
            postAnimationGraphables.capacity = adapter.data.capacity
...
//        }
    ...
    private fun populateRenderingPath() {
        adapter.data.useLock {  // use lock during mutation of graphables
            ...
            // update the capacity in case it changed
            postAnimationGraphables.capacity = adapter.data.capacity
            rawExtraGraphableList?.let { scaleGraphables(it, postAnimationExtraGraphables) }
...
//                }
    ...
    private fun rasterizeClipFillPath(renderPoints: MutableList<Graphable>) {
        if (adapter.axisArray.isEmpty()) return
...
//        fillPaint.shader = HueGradientShader.generate(viewHeight)
    ...
    fun drawExtra(canvas: Canvas) {
        val paint = adapter.axisArray[0].paint
        val textPaint = adapter.axisArray[0].textPaint
        fun drawHorizontalLine(canvas: Canvas, g: DrawingGraphable, viewDimensions: ViewDimensions, getAlignment: (List<PointGraphable>, MutableList<PointGraphable>) -> Unit) {
            val renderPointsOut = arrayListOf<PointGraphable>()
            getAlignment(listOf(
                PointGraphable(0f, g.pointY),
                PointGraphable(viewDimensions.width, g.pointY)
            ), renderPointsOut)

            renderPointsOut[1].x = viewDimensions.width
            canvas.drawLine(renderPointsOut[0].x, renderPointsOut[0].y, renderPointsOut[1].x, renderPointsOut[1].y, paint)
            canvas.drawText(g.label, renderPointsOut[1].x - 50, renderPointsOut[1].y, textPaint)
        }

        adapter.drawings?.forEach {
            drawHorizontalLine(canvas, it, viewDimensions) { points, renderPointsOut ->
    ...
    fun draw(
        ...
        graphSelectionRange: GraphRangeSelector
    ) {
        // update the capacity in case it changed
        postAnimationGraphables.capacity = adapter.data.capacity
        adapter.axisArray.forEach {
        ...
        if (snapToNearest && postAnimationGraphables.isNotEmpty() && adapter.selectionRange.value?.redraw == true) {
            val range = adapter.selectionRange.value ?: return
            val x0 = range.start
            val x1 = range.end
            graphSelectionRange.startX = postAnimationGraphables[x0].x
            graphSelectionRange.endX = postAnimationGraphables[x1].x
            range.redraw = false
        }
